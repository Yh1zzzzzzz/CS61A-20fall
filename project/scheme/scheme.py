...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(oper,oprands,env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        current = self
        falg = False
        while current != None and  not falg:
            if symbol in current.bindings.keys():
                falg = True
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        expression = Pair(para , func_body)
        temp = do_lambda_form(expression,env)
        env.define(fun_name,temp)
        return fun_name
